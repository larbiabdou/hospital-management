...
class ResPartner(models.Model):
    ...
    def _compute_age(self):
        for record in self:
            if record.date_of_birth:
                ...
                # Calcul de l'âge en tenant compte des années bissextiles
    ...
    def search(self, args, offset=0, limit=None, order=None):
        custom_args = ['|', ('name', 'ilike', args[0]), ('patient_seq', 'ilike', args[0])]
        return super(ResPartner, self).search(custom_args, offset=offset, limit=limit, order=order)

    def compute_appointment_count(self):
        for record in self:
            record.appointment_count = self.env['hospital.outpatient'].search_count([('patient_id', '=', record.id), ('state', '!=', 'draft')])
